...
class TakTunnelRotateRouteCheckFailure(RuntimeError):
    pass
...
def load_fixture() -> tuple[User, int, int]:
    with SessionLocal() as db:
        ...
        if admin is None or tic is None or tak is None:
            raise TakTunnelRotateRouteCheckFailure("missing admin/tic/tak seed data")
        db.expunge(admin)
...
def run() -> None:
    ...
    def fake_rotate(db, actor, *, tic_server_id: int, tak_server_id: int) -> None:
        captured.append((tic_server_id, tak_server_id))

    with patch("app.web.rotate_tak_tunnel_pair", side_effect=fake_rotate), patch(
    ...
    if response.status_code != 200:
        ...
        raise TakTunnelRotateRouteCheckFailure(
            f"rotate route returned {response.status_code}: {detail}"
        )
    if captured != [(tic_id, tak_id)]:
        raise TakTunnelRotateRouteCheckFailure(
            f"rotate route called service with unexpected args: {captured!r}"
        )
    if "stub diagnostics" not in response.text:
        raise TakTunnelRotateRouteCheckFailure("rotate route did not render diagnostics response")
    print("OK: tak tunnel rotate route check passed")


if __name__ == "__main__":
    try:
        run()
    except TakTunnelRotateRouteCheckFailure as exc:
