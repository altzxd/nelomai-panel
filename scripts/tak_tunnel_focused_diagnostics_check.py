...
class TakTunnelFocusedDiagnosticsCheckFailure(RuntimeError):
    pass
...
def load_fixture() -> tuple[User, int, int]:
    with SessionLocal() as db:
        ...
        if admin is None or tic is None or tak is None:
            raise TakTunnelFocusedDiagnosticsCheckFailure("missing admin/tic/tak seed data")
        db.expunge(admin)
...
def stub_page(tic_id: int) -> DiagnosticsPageView:
    return DiagnosticsPageView(
        ...
        summary="focused diagnostics stub",
        ...
        focused_tak_tunnel=DiagnosticsFocusedTakTunnelView(
            ...
            details=[
                "Статус агента: cooldown",
                "Ревизия артефактов: 3",
                "Последняя ротация артефактов: 2026-05-03T12:00:00+00:00",
                "Повторная попытка не раньше: 2026-05-03T12:10:00+00:00",
            ],
...
def run() -> None:
    ...
    if response.status_code != 200:
        ...
        raise TakTunnelFocusedDiagnosticsCheckFailure(
    ...
    required_tokens = [
        ...
        "Статус агента: cooldown",
        "Ревизия артефактов: 3",
        "Последняя ротация артефактов:",
        "Логи автовосстановления",
        "Логи cooldown",
        "Логи ручного внимания",
        "Логи ручного восстановления",
    ...
    if missing:
        raise TakTunnelFocusedDiagnosticsCheckFailure(
            "focused diagnostics misses tokens: " + ", ".join(missing)
        )
    print("OK: tak tunnel focused diagnostics check passed")


if __name__ == "__main__":
    try:
        run()
    except TakTunnelFocusedDiagnosticsCheckFailure as exc:
