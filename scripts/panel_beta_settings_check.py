...
class PanelBetaSettingsCheckFailure(RuntimeError):
    pass
...
def load_admin() -> User:
    with SessionLocal() as db:
        ...
        if admin is None:
            raise PanelBetaSettingsCheckFailure("missing admin seed data")
        db.expunge(admin)
        return admin
    ...
    return AdminPageView(
        ...
        beta_readiness=BetaReadinessSummaryView(
            ...
            details=[
                ...
                "Не задан PEER_AGENT_COMMAND для server-side действий.",
            ],
...
def run() -> None:
    ...
    with patch("app.web.get_admin_page_data", return_value=stub_page()):
        with TestClient(app) as client:
            response = client.get("/admin?tab=settings&settings_view=basic", headers=auth_headers(admin))

    if response.status_code != 200:
        detail = response.text[:500].replace("\n", " ")
        raise PanelBetaSettingsCheckFailure(
            f"/admin?tab=settings&settings_view=basic returned {response.status_code}: {detail}"
        )
    ...
    required_tokens = [
        ...
        "Не задан NELOMAI_GIT_REPO для deploy/bootstrap/update path.",
        "Не задан PEER_AGENT_COMMAND для server-side действий.",
        "Порядок исправления production-blocker",
    ...
    if missing:
        raise PanelBetaSettingsCheckFailure("beta settings misses tokens: " + ", ".join(missing))
    print("OK: panel beta settings check passed")


if __name__ == "__main__":
    try:
        run()
    except PanelBetaSettingsCheckFailure as exc:
